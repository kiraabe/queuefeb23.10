...
export async function initDb() {
  ...
  try {
    ...
    // Add standard_time_minutes column to services if it doesn't exist
    await p.query(
      `ALTER TABLE services ADD COLUMN IF NOT EXISTS standard_time_minutes int;`,
    );

    // Migrate data from standard_time_seconds to standard_time_minutes if needed
    await p.query(
      `ALTER TABLE services DROP COLUMN IF EXISTS standard_time_seconds;`,
    );
...
async function checkAndResetExpiredTickets(
  ...
  try {
    // Find tickets older than 24 hours that aren't already marked as done/skipped/on_hold
    // NOTE: Exclude field_visit and on_hold status tickets
    // - field_visit tickets persist across days as part of multi-day field work
    // - on_hold tickets are handled by autoCancelExpiredHolds (72-hour rule)
    const expiredRes = await client.query(
      `SELECT COUNT(*) as count FROM tickets
       WHERE created_at < $1 AND status NOT IN ('done', 'skipped', 'field_visit', 'on_hold')`,
      [expirationThreshold],
    );
    ...
    if (expiredCount > 0) {
      ...
      // NOTE: field_visit and on_hold status tickets are excluded from this 24-hour expiration
      // - field_visit tickets persist across days as part of multi-day field work
      // - on_hold tickets are handled separately by autoCancelExpiredHolds (72-hour rule)
      await client.query(
        `UPDATE tickets
...
export async function skipDb(windowId: number, reason?: string) {
  ...
  try {
    ...
    // Update teller progress record to mark as skipped
    await client.query(
      `UPDATE employee_case_performance SET ended_at=now(), status='skipped' WHERE ticket_id=$1 AND step_type='teller' AND window_id=$2 AND status='in_progress'`,
      [w.currentTicketId, windowId],
    );
...
export async function getServicesByCategoryDb(categoryId: string) {
  ...
  const { rows: serviceRows } = await p.query(
    `SELECT id, category_id, code, name, standard_time_minutes, extract(epoch from created_at)*1000 as created_at, extract(epoch from updated_at)*1000 as updated_at
  ...
  return {
    ...
    services: serviceRows.map((r) => ({
      ...
      code: r.code,
      name: r.name,
      standardTimeMinutes: r.standard_time_minutes || undefined,
      createdAt: Math.round(Number(r.created_at)),
      updatedAt: Math.round(Number(r.updated_at)),
    })),
  };
...
export async function createServiceDb(params: {
  ...
  const p = getPool();
  const { rows } = await p.query(
    `INSERT INTO services (id, category_id, code, name, standard_time_minutes)
     VALUES (gen_random_uuid(), $1, $2, $3, $4)
     RETURNING id, category_id, code, name, standard_time_minutes`,
    [
      params.categoryId,
      params.code,
      params.name,
      params.standardTimeMinutes || null,
    ],
  );
  ...
  return {
    ...
    code: rows[0].code,
    name: rows[0].name,
    standardTimeMinutes: rows[0].standard_time_minutes || undefined,
  };
}

export async function updateServiceDb(
  serviceId: string,
  params: { code?: string; name?: string; standardTimeMinutes?: number },
): Promise<{
  id: string;
  categoryId: string;
  code: string;
  ...
  if (params.standardTimeMinutes !== undefined) {
    updates.push(`standard_time_minutes = $${paramCount}`);
    values.push(params.standardTimeMinutes || null);
    paramCount++;
  }
