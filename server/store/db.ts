...
export async function initDb() {
  ...
  try {
    ...
    // Seed demo users and test cases for development
    try {
      // Create a test employee user (Almaz Kebede)
      const { createHash } = await import("node:crypto");
      const hashPassword = (pwd: string) => {
        return createHash("sha256").update(pwd).digest("hex");
      };

      const employeeId = (await import("node:crypto")).randomUUID();
      const testEmployeePassword = hashPassword("employee123");

      // Upsert test employee
      await p.query(
        `INSERT INTO users (id, username, password_hash, role, full_name, department, email)
      ...
      console.log("✅ Test employee created: username='almaz.kebede', password='employee123'");

      // Get the actual employee ID (in case of conflict, use existing)
      const empRes = await p.query(
        `SELECT id FROM users WHERE username = 'almaz.kebede'`,
      );
      const finalEmployeeId = empRes.rows[0]?.id;

      if (finalEmployeeId) {
        // Create test cases transferred to this employee
        const cadastralCategoryRes = await p.query(
          `SELECT id FROM service_categories WHERE code = 'cadastral-group'`,
        );
        const categoryId = cadastralCategoryRes.rows[0]?.id;

        if (categoryId) {
          // Get some service IDs from cadastral category
          const servicesRes = await p.query(
            `SELECT id FROM services WHERE category_id = $1 LIMIT 2`,
            [categoryId],
          );
          const serviceIds = servicesRes.rows.map((r: any) => r.id);
   ...
   - 2 Received (CG-001, CG-002)
   - 1 Completed (CG-003)`);
        }
      }
    } catch (error) {
      console.log(
        "ℹ️  Note: Could not seed test data (this is normal in production)",
      );
    }

    // Admin user creation has been removed - users must be created explicitly through the setup/management API
    console.log("ℹ️  Database initialization complete.");
  } catch (error) {
    throw error;
  }
}
